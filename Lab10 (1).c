...
typedef struct dijkstra_st dijkstra_t;

// FUNÇÂO QUE CALCULA CAMINHO MÍNIMO PELO ALGORISMO DE DIJKSTRA
void dijkstra(nodo_t *nodos, int *arestas, int origem, int destino, int n){
  int i, j, visita;
  ...
  do{
    for(j=0;j<n;j++){
      if(arestas[origem][j]!=-1){
	caminho[i].dist=aresta[i][j];
